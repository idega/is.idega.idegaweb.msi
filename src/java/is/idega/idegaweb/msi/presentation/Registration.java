/*
 * $Id: Registration.java,v 1.1 2007/06/07 22:54:34 palli Exp $
...
import is.idega.idegaweb.msi.business.Runner;
...
public class Registration extends RaceBlock {
	...
	private void stepPersonalDetails(IWContext iwc) throws RemoteException {
		...
		choiceTable.add(getHeader(localize(MSIConstants.RR_PRIMARY_DD, "Run") + "/" + localize(MSIConstants.RR_SECONDARY_DD, "Distance")), 1, iRow);
		...
		Collection countries = getRaceBusiness(iwc).getCountries();
		...
		choiceTable.add(getHeader(localize(MSIConstants.RR_SSN, "SSN")), 1, iRow);
		choiceTable.add(redStar, 1, iRow);
		choiceTable.add(getHeader(localize(MSIConstants.RR_NATIONALITY, "Nationality")), 3, iRow);
		...
		choiceTable.add(getHeader(localize(MSIConstants.RR_ADDRESS, "Address")), 1, iRow);
		choiceTable.add(redStar, 1, iRow);
		choiceTable.add(getHeader(localize(MSIConstants.RR_EMAIL, "Email")), 3, iRow++);
		...
		choiceTable.add(getHeader(localize(MSIConstants.RR_CITY, "City")), 1, iRow);
		choiceTable.add(redStar, 1, iRow);
		choiceTable.add(getHeader(localize(MSIConstants.RR_TEL, "Telephone")), 3, iRow++);
		...
		choiceTable.add(getHeader(localize(MSIConstants.RR_POSTAL, "Postal Code")), 1, iRow);
		choiceTable.add(redStar, 1, iRow);
		choiceTable.add(getHeader(localize(MSIConstants.RR_MOBILE, "Mobile Phone")), 3, iRow++);
		...
		choiceTable.add(getHeader(localize(MSIConstants.RR_COUNTRY, "Country")), 1, iRow);
		choiceTable.add(redStar, 1, iRow);
		choiceTable.add(getHeader(localize(MSIConstants.RR_TSHIRT, "T-Shirt")), 3, iRow);
		choiceTable.add(redStar, 3, iRow++);
	...
	private void stepChip(IWContext iwc) {
		...
		RadioButton rentChip = getRadioButton(PARAMETER_CHIP, MSIConstants.CHIP_RENT);
		...
		RadioButton ownChip = getRadioButton(PARAMETER_CHIP, MSIConstants.CHIP_OWN);
		...
		RadioButton buyChip = getRadioButton(PARAMETER_CHIP, MSIConstants.CHIP_BUY);
	...
	private void stepReceipt(IWContext iwc) throws RemoteException {
		...
		while (iter.hasNext()) {
			...
			runnerTable.add(getText(localize(runner.getRun().getName(), runner.getRun().getName())), 2, runRow);
			runnerTable.add(getText(localize(runner.getDistance().getName(), runner.getDistance().getName())), 3, runRow);
			float runPrice = getRaceBusiness(iwc).getPriceForRunner(runner, iwc.getCurrentLocale(), this.chipDiscount, 0);
		...
		Help help = new Help();
		help.setHelpTextBundle(MSIConstants.IW_BUNDLE_IDENTIFIER);
	...
	private void save(IWContext iwc, boolean doPayment) throws RemoteException {
		try {
			...
			if (doPayment) {
				properties = getRaceBusiness(iwc).authorizePayment(nameOnCard, cardNumber, expiresMonth, expiresYear, ccVerifyNumber, amount, this.isIcelandic ? "ISK" : "EUR", referenceNumber);
			}
			Collection participants = getRaceBusiness(iwc).saveParticipants(runners, email, hiddenCardNumber, amount, paymentStamp, iwc.getCurrentLocale());
			if (doPayment) {
	...
	private Runner collectValues(IWContext iwc) throws FinderException, RemoteException {
		...
		if (personalID != null && personalID.length() > 0) {
			...
			if (iwc.isParameterSet(PARAMETER_RUN)) {
				runner.setRun(ConverterUtility.getInstance().convertGroupToRace(new Integer(iwc.getParameter(PARAMETER_RUN))));
			}
			if (iwc.isParameterSet(PARAMETER_DISTANCE)) {
				runner.setDistance(ConverterUtility.getInstance().convertGroupToRaceEvent(new Integer(iwc.getParameter(PARAMETER_DISTANCE))));
			}
			...
			if (iwc.isParameterSet(PARAMETER_CHIP)) {
				...
				if (chip.equals(MSIConstants.CHIP_RENT)) {
					runner.setRentChip(true);
				}
				else if (chip.equals(MSIConstants.CHIP_BUY)) {
					runner.setBuyChip(true);
				}
				else if (chip.equals(MSIConstants.CHIP_OWN)) {
		...
		if (action == ACTION_STEP_DISCLAIMER) {
			if (this.runner != null && this.runner.isOwnChip() && (this.runner.getChipNumber() == null || !checkChipNumber(this.runner.getChipNumber()).equals(""))) {
				...
				action = ACTION_STEP_CHIP;
					...
					//Season year = this.runner.getDistance().getYear();
			}
		}
