...
public class RunYearTab extends UserGroupTab{
	...
	public void initFieldContents() {
		try {
			Season year = ConverterUtility.getInstance().convertGroupToSeason(new Integer(getGroupId()));
			
/*			if (year.getRaceDate() != null) {
				this.fieldValues.put(PARAMETER_RUN_DATE, year.getRaceDate());
			}
	...
	public boolean store(IWContext iwc) {
		try {
			if (getGroupId() > -1) {
				...
				//year.setRaceDate((Timestamp) this.fieldValues.get(PARAMETER_RUN_DATE));
