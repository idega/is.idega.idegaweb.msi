...
public class UserRunTab extends UserTab{
	...
	public void lineUpFields() {
		...
		/*try {			
			this.runGroups = null;//runBiz.getRunGroupsForUser(user);
		}
		...
		if(this.runGroups != null) {
			...
			while(i.hasNext()) {
				...
				Group run = null;
				/*try {
					run = null;//runBiz.getRunGroupOfTypeForGroup(runGroup, MSIConstants.GROUP_TYPE_RACE);
				}
				catch (RemoteException e) {
					e.printStackTrace();
				}*/
				Group year = null;
				/*try {
					year = null;//runBiz.getRunGroupOfTypeForGroup(runGroup, MSIConstants.GROUP_TYPE_SEASON);
				}
				catch (RemoteException e) {
					e.printStackTrace();
				}*/
				...
				l.addParameter(MSIConstants.GROUP_TYPE_RACE,run.getPrimaryKey().toString());
